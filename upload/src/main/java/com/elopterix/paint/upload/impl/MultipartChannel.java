...
class MultipartChannel {
     ...
     */
    public MultipartChannel(InputStream input, byte[] boundary) {
        ...
        this.readableChannel = Channels.newChannel(input);
        ...
        if (byteBuffer.capacity() < this.boundaryLength + 1) {
    ...
    private void noteBytesRead(long additional) {
    ...
    byte readByte() throws IOException {
        ...
        if (head == tail) {
            head = 0;
            // Refill.
            tail = input.read(byteBuffer.array(), head, byteBuffer.capacity());
            if (tail == -1) {
     ...
     * @throws IOException if the stream ends unexpectedly or
    ...
    public boolean readBoundary() throws IOException {
        ...
        try {
            marker[0] = readByte();
            marker[1] = readByte();
            if (marker[0] == STREAM_TERMINATOR[0] && marker[1] == STREAM_TERMINATOR[1]) {
                nextChunk = false;
            } else if (marker[0] == FIELD_SEPARATOR[0] && marker[1] == FIELD_SEPARATOR[1]) {
                nextChunk = true;
            } else {
     ...
     * @throws IOException    if the bytes read from the stream exceeded the size limits.
    ...
    ReadableByteChannel newItemChannel() {
        return Channels.newChannel(new ItemInputStream());
    }
    ...
    public int discardBodyData() throws IOException {
        try (ReadableByteChannel input = newItemChannel()) {
            int total = 0;
            int len;
    ...
    int findSeparator() {
        ...
        for (first = head; first <= maxpos && match != boundaryLength; first++) {
            ...
            for (match = 1; match < boundaryLength; match++) {
                if (byteBuffer.get(first + match) != boundary[match]) {
                    break;
                }
            }
        }
    ...
    public class ItemInputStream extends InputStream {
        ...
        private void findSeparator() {
            pos = MultipartChannel.this.findSeparator();
            if (pos == -1) {
        ...
        public int read(@Nonnull byte[] b, int off, int len) throws IOException {
            ...
            System.arraycopy(byteBuffer.array(), head, b, off, res);
            head += res;
            total += res;
            return res;
        }
        ...
        private int makeAvailable() throws IOException {
            ...
            total += tail - head - pad;
            System.arraycopy(byteBuffer.array(), tail - pad, byteBuffer.array(), 0, pad);
            ...
            for (; ; ) {
                int bytesRead = input.read(byteBuffer.array(), tail, byteBuffer.capacity() - tail);
                if (bytesRead == -1) {
